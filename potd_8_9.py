...
class Solution:
    ...
    def getnodes(self,root,arr):
        if root == None:
            return
        self.getnodes(root.left,arr)
        arr.append(root.data)
        self.getnodes(root.right,arr)
        
    def binaryTreeToBST(self, root):
        arr = []
        self.getnodes(root,arr)
        # print(arr)
        arr.sort()
        i = 0
        #sorted array to binary search tree
        def inorder_bst(root,arr):
            nonlocal i
            if root == None:
                return 
            inorder_bst(root.left,arr)
            root.data = arr[i]
            i += 1
